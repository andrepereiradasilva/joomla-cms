...
class JAccess
{
	...
	/**
	 * Method to return the calculated permissions.
	 *
	 * @param   integer  $assetId        The current asset id.
	 * @param   integer  $assetParentId  The current asset parent id.
	 * @param   integer  $groupId        The current group id.
	 * @param   integer  $groupParentId  The current group parent id.
	 * @param   string   $action         The action (ex: core.edit).
	 *
	 * @return  array   Array with allowed, locked and text.
	 *
	 * @since   __DEPLOY_VERSION__
	 */
	public static function getCalculatedPermission($assetId, $assetParentId, $groupId, $groupParentId, $action)
	{
		// Sanitize.
		$assetId       = (int) $assetId;
		$assetParentId = (int) $assetParentId;
		$groupId       = (int) $groupId;
		$groupParentId = (int) $groupParentId;

		// Current group is a Super User group, so calculated setting is "Allowed (Super User)".
		if (self::checkGroup($groupId, 'core.admin'))
		{
			return array('allowed' => 1, 'locked' => 1, 'text' => JText::_('JLIB_RULES_ALLOWED_ADMIN'));
		}

		// Get the rule for just this asset (non-recursive) and get the actual setting for the action for the group.
		$ruleValue = self::getAssetRules($assetId, false, false)->allow($action, $groupId);

		// Global configuration with "Not Set" permission. Calculated permission is "Not Allowed (Default)".
		if ($groupParentId === 0 && $ruleValue === null)
		{
			return array('allowed' => 0, 'locked' => 0, 'text' => JText::_('JLIB_RULES_NOT_ALLOWED_DEFAULT'));
		}

		// Get the group, group parent id, and group global config recursive calculated permission for the chosen action.
		$inheritedGroupRule            = self::checkGroup($groupId, $action, $assetId);
		$inheritedGroupParentAssetRule = $assetParentId ? self::checkGroup($groupId, $action, $assetParentId) : null;
		$inheritedParentGroupRule      = $groupParentId ? self::checkGroup($groupParentId, $action, $assetId) : null;
		 ...
		 */
		if ($inheritedGroupParentAssetRule === false || $inheritedParentGroupRule === false)
		{
			return array('allowed' => 0, 'locked' => 1, 'text' => JText::_('JLIB_RULES_NOT_ALLOWED_LOCKED'));
		}

		// If there is an explicit permission is "Allowed". Calculated permission is "Allowed".
		if ($ruleValue === true)
		{
			return array('allowed' => 1, 'locked' => 0, 'text' => JText::_('JLIB_RULES_ALLOWED'));
		}

		// If there is an explicit permission "Not Allowed". Calculated permission is "Not Allowed".
		if ($ruleValue === false)
		{
			return array('allowed' => 0, 'locked' => 0, 'text' => JText::_('JLIB_RULES_NOT_ALLOWED'));
		}

		// If recursive calculated setting is "Denied" or null. Calculated permission is "Not Allowed (Inherited)".
		if ($inheritedGroupRule === null || $inheritedGroupRule === false)
		{
			return array('allowed' => 0, 'locked' => 0, 'text' => JText::_('JLIB_RULES_NOT_ALLOWED_INHERITED'));
		}

		// If recursive calculated setting is "Allowed". Calculated permission is "Allowed (Inherited)".
		return array('allowed' => 1, 'locked' => 0, 'text' => JText::_('JLIB_RULES_ALLOWED_INHERITED'));
	}
}
